...
3. **After implementation** — Update `docs/PROJECT_STATUS.md` checklists (mark items complete, update known issues, add to release history). **Always recount** all `- [x]` and `- [ ]` items across the entire file to recalculate category totals and the Overall Progress section (progress bar, percentage, done/total counts) — never do incremental math on stale numbers. When adding a release to `PROJECT_STATUS.md §10`, also add the corresponding entry to `CHANGELOG.md` (Keep a Changelog format).
